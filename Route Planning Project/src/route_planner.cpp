...
RouteModel::Node *RoutePlanner::NextNode() {
    ...
    int count = 0;
    int track = 0;
    RouteModel::Node* lowestNode = open_list[0];

    for (RouteModel::Node * node : open_list ){
        if( (lowestNode->g_value + lowestNode->h_value) > (node->g_value + node->h_value)  ){
            lowestNode = node;
            track = count;
        }
        count++;
    ...
    open_list.erase(open_list.begin()+track);
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    std::cout <<"after stuck"<<"\n";
    path_found.push_back(*start_node);
    distance += node->distance(*start_node);
